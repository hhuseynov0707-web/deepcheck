...
def fetch_history(api, operator_key, session_id):
    request = urllib.request.Request(
        f"{api}/api/score/{session_id}", headers={"X-Dashboard-Key": operator_key}
    )
...
def main():
    ...
    parser.add_argument("--operator-key", default="deepcheck-dev-pano-anahtari-2026")
    ...
    try:
        with sync_playwright() as playwright:
            ...
            # installed. The hard-coded Linux path this used to carry only
            # existed in the container the lab was written in.
