...
def chromium_launch_args(headless: bool = True) -> dict:
    ...
    PW_CHROMIUM overrides the binary when it points at one that exists;
    otherwise Playwright uses the browser it installed itself. The previous
    hard-coded "/opt/pw-browsers/chromium" was the path inside the Linux
    container this lab was written in, and it made the lab unrunnable
    anywhere else.
    """
    launch = {"headless": headless}
    override = os.environ.get("PW_CHROMIUM")
...
def main():
    ...
    launch = chromium_launch_args(headless=not args.headed)
